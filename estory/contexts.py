...
class Ctx(conv.State):
    ...
    def __init__(self, req = None):
        if req is not None:
            self.req = req
            estory_env = req.environ.get('e-story', {})
            for key in object.__getattribute__(self, 'env_keys'):
                value = estory_env.get(key)
    ...
    def blank_req(self, path, environ = None, base_url = None, headers = None, POST = None, **kw):
        ...
        estory_env = env.setdefault('e-story', {})
        for key in self.env_keys:
            value = getattr(self, key)
            if value is not None:
                estory_env[key] = value
    ...
    def lang_del(self):
        del self._lang
        if self.req is not None and self.req.environ.get('e-story') is not None \
                and '_lang' in self.req.environ['e-story']:
            del self.req.environ['e-story']['_lang']

    def lang_get(self):
        if self._lang is None:
            self._lang = ['en-US', 'en']
            if self.req is not None:
                self.req.environ.setdefault('e-story', {})['_lang'] = self._lang
        return self._lang
    ...
    def lang_set(self, lang):
        ...
        if self._translator is not None:
            ...
            if self.req is not None and self.req.environ.get('e-story') is not None \
                    and '_translator' in self.req.environ['e-story']:
                del self.req.environ['e-story']['_translator']
    ...
    def node_del(self):
        del self._node
        if self.req is not None and self.req.environ.get('e-story') is not None \
                and '_node' in self.req.environ['e-story']:
            del self.req.environ['e-story']['_node']
    ...
    def node_set(self, node):
        self._node = node
        if self.req is not None:
            self.req.environ.setdefault('e-story', {})['_node'] = node
    ...
    def session_del(self):
        del self._session
        if self.req is not None and self.req.environ.get('e-story') is not None \
                and '_session' in self.req.environ['e-story']:
            del self.req.environ['e-story']['_session']
    ...
    def session_set(self, session):
        ...
        if self.req is not None:
            self.req.environ.setdefault('e-story', {})['_session'] = session
    ...
    def user_del(self):
        del self._user
        if self.req is not None and self.req.environ.get('e-story') is not None \
                and '_user' in self.req.environ['e-story']:
            del self.req.environ['e-story']['_user']
    ...
    def user_set(self, user):
        self._user = user
        if self.req is not None:
            self.req.environ.setdefault('e-story', {})['_user'] = user
