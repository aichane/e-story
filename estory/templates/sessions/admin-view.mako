...
%>\
    % if lesson is None:
        <div class="field">
            <b class="field-label">${_('{0}:').format(_("Lesson"))}</b>
            <span class="field-value">${session.lesson_id}</span>
        </div>
    % else:
        <div class="field">
            <b class="field-label">${_('{0}:').format(_("Lesson"))}</b>
            <a class="field-value" href="${lesson.get_admin_url(ctx)}">${lesson.get_title(ctx)}</span></a>
        </div>
    % endif
        <div class="field">
