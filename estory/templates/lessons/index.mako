...
<%inherit file="/object-index.mako"/>
...
<%def name="container_content()" filter="trim">
    ...
    % else:
        ...
        <table class="table table-bordered table-condensed table-striped">
            ...
            % else:
                    <th><a href="${model.Lesson.get_class_url(ctx, **urls.relative_query(inputs, page = None,
...
%>\
                    ...
                    <td>
                        <h4><a href="${lesson.get_url(ctx)}">${lesson.title}</a></h4>
<%
...
<%def name="search_form()" filter="trim">
        <form action="${model.Lesson.get_class_url(ctx)}" method="get" role="form">
...
##                    advanced_search = None, format = 'atom', page = None, sort = None))}">${_('News Feed')}</a>
    % if data['advanced_search']:
            <a class="pull-right" href="${model.Lesson.get_class_url(ctx, **urls.relative_query(inputs,
