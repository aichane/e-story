...
<%def name="container_content()" filter="trim">
        <h2>${_(u'Deletion of {}').format(item.get_title(ctx))}</h2>
        <p class="confirm">${_(u"Are you sure that you want to delete this item?")}</p>
        <form method="post" action="${item.get_admin_url(ctx, 'delete')}">
            <%view:view_fields/>
...
${_(u'Delete')} - ${item.get_title(ctx)} - ${parent.title_content()}
</%def>
