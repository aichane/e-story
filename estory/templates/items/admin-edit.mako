...
<%def name="breadcrumb_content()" filter="trim">
            ...
            <li><a href="${item.get_admin_url(ctx)}">${item.get_title(ctx)}</a></li>
            <li class="active">${_(u'Edit')}</li>
</%def>


<%def name="container_content()" filter="trim">
        <form action="${item.get_admin_url(ctx, 'edit')}" method="post" role="form">
            <%self:hidden_fields/>
            <fieldset>
                <legend>${_(u'Edition of %s') % item.get_title(ctx)}</legend>
                <%self:error_alert/>
                <%self:form_fields/>
...
%>\
                <div class="form-group${' has-error' if error else ''}">
                    ...
                    <input class="form-control" id="url" name="url" type="url" value="${inputs['url'] or ''}">
    % if error:
                    <span class="help-block">${error}</span>
    % endif
                </div>
<%
    error = errors.get('content') if errors is not None else None
%>\
                <div class="form-group${' has-error' if error else ''}">
                    <label for="content">${_("HTML Fragment")}</label>
                    <textarea class="form-control" id="content" name="content">${inputs['content'] or ''}</textarea>
    % if error:
                    <span class="help-block">${error}</span>
    % endif
                </div>
<%
    error = errors.get('temporal_coverage_from') if errors is not None else None
%>\
                <div class="form-group${' has-error' if error else ''}">
                    <label for="temporal_coverage_from">${_("Temporal Coverage From")}</label>
                    <input class="form-control" id="temporal_coverage_from" name="temporal_coverage_from" required type="text" value="${inputs['temporal_coverage_from'] or ''}">
    % if error:
                    <span class="help-block">${error}</span>
    % endif
                </div>
<%
    error = errors.get('temporal_coverage_to') if errors is not None else None
%>\
                <div class="form-group${' has-error' if error else ''}">
                    <label for="temporal_coverage_to">${_("Temporal Coverage To")}</label>
                    <input class="form-control" id="temporal_coverage_to" name="temporal_coverage_to" type="text" value="${inputs['temporal_coverage_to'] or ''}">
    % if error:
                    <span class="help-block">${error}</span>
    % endif
                </div>
...
${_(u'Edit')} - ${item.get_title(ctx)} - ${parent.title_content()}
</%def>
